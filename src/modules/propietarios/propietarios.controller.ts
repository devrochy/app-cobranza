import { Body, Controller, Get, Param, ParseIntPipe, Patch, Post, Put, Query, Req, UseGuards } from "@nestjs/common";
import type { Request } from "express";
import { JwtAuthGuard } from "../auth/jwt-auth.guard";
import { PermisoGuard } from "../auth/permiso.guard";
import { PermisoRequerido } from "../auth/permiso-requerido.decorator";
import { AuthTokenPayload } from "../auth/auth.service";
import { CreatePropietarioDto } from "./dto/create-propietario.dto";
import { ListarPropietariosDto } from "./dto/listar-propietarios.dto";
import { UpdateEstatusDto } from "./dto/update-estatus.dto";
import { UpdatePermisosDto } from "./dto/update-permisos.dto";
import { UpdatePropietarioDto } from "./dto/update-propietario.dto";
import { ActualizarConfiguracionPropietarioDto } from "./dto/actualizar-configuracion-propietario.dto";
import { PermisosPropietarioService } from "./permisos-propietario.service";
import { PropietariosService } from "./propietarios.service";

@Controller("propietarios")
export class PropietariosController {
  constructor(
    private readonly propietariosService: PropietariosService,
    private readonly permisosPropietarioService: PermisosPropietarioService,
  ) {}

  @Post()
  @PermisoRequerido("registrar_propietario")
  @UseGuards(JwtAuthGuard, PermisoGuard)
  create(@Body() dto: CreatePropietarioDto) {
    return this.propietariosService.create(dto);
  }

  @Patch(":id")
  @UseGuards(JwtAuthGuard, PermisoGuard)
  update(@Param("id", ParseIntPipe) id: number, @Body() dto: UpdatePropietarioDto) {
    return this.propietariosService.update(id, dto);
  }

  @Get()
  @PermisoRequerido("ver_reportes")
  @UseGuards(JwtAuthGuard, PermisoGuard)
  listar(
    @Query() query: ListarPropietariosDto,
    @Req() req: Request & { user: AuthTokenPayload },
  ) {
    return this.propietariosService.listar(query, {
      rol: req.user.rol,
      sub: req.user.sub,
    });
  }

  @Get(":id")
  @UseGuards(JwtAuthGuard, PermisoGuard)
  getById(@Param("id", ParseIntPipe) id: number) {
    return this.propietariosService.obtener(id);
  }

  @Patch(":id/configuracion")
  @PermisoRequerido("editar_configuracion_propietario")
  @UseGuards(JwtAuthGuard, PermisoGuard)
  actualizarConfiguracion(
    @Param("id", ParseIntPipe) id: number,
    @Body() dto: ActualizarConfiguracionPropietarioDto,
    @Req() req: Request & { user: AuthTokenPayload },
  ) {
    return this.propietariosService.actualizarConfiguracion(id, dto, {
      rol: req.user.rol,
      sub: req.user.sub,
    });
  }

  @Patch(":id/estatus")
  @UseGuards(JwtAuthGuard, PermisoGuard)
  setEstatus(@Param("id", ParseIntPipe) id: number, @Body() dto: UpdateEstatusDto) {
    // Admin-only por decisión de HU-07: `bloquear_propietario` no es accesible para
    // propietarios (evita DoS cross-tenant); el catálogo lo conserva para semántica futura.
    return this.propietariosService.setEstatus(id, dto.estatus);
  }

  @Get(":id/permisos")
  @UseGuards(JwtAuthGuard, PermisoGuard)
  getPermisos(@Param("id", ParseIntPipe) id: number) {
    return this.permisosPropietarioService.getMatriz(id);
  }

  @Put(":id/permisos")
  @UseGuards(JwtAuthGuard, PermisoGuard)
  setPermisos(@Param("id", ParseIntPipe) id: number, @Body() dto: UpdatePermisosDto) {
    return this.permisosPropietarioService.setMatriz(id, dto.matriz);
  }
}
