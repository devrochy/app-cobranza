...
export class ListaClientesDelDiaService {
  ...
  async obtener(carteraId: number, requester: RequesterListaDiaContext): Promise<ClienteDiaPublic[]> {
    ...
    try {
      const planificado = await this.carteraOptimizacionService.consultar(carteraId, requester);
    ...
    const clientes = await this.listarClientesConEstado(carteraId);
    const pagaronHoy = new Set(await this.clientesConVisitaPagoHoy(carteraId));
  ...
  async obtenerMapa(carteraId: number, requester: RequesterListaDiaContext): Promise<MarkerClientePublic[]> {
    const clientes = await this.obtener(carteraId, requester);
    const coords = await this.coordenadasDeClientes(carteraId);
  ...
  private async clientesConVisitaPagoHoy(carteraId: number): Promise<number[]> {
    const hoy = this.fechaLocal(new Date());
