...
describe("LiquidacionesService", () => {
  ...
  it("un propietario no puede liquidar una cartera ajena -> 403", async () => {
...
describe("LiquidacionesService - historial y exportación", () => {
  ...
  it("listar lanza NotFoundException si la cartera no existe", async () => {
    (carteraRepo.findOne as jest.Mock).mockResolvedValue(null);

    await expect(service.listar(999, adminContext)).rejects.toThrow(NotFoundException);
  });

  it("listar devuelve las liquidaciones de la cartera ordenadas por fecha DESC", async () => {
    (carteraRepo.findOne as jest.Mock).mockResolvedValue(carteraFixture());
  ...
  it("un propietario no puede listar una cartera ajena -> 403", async () => {
    (carteraRepo.findOne as jest.Mock).mockResolvedValue(carteraFixture({ propietarioId: 2 }));

    await expect(service.listar(1, propietarioContext)).rejects.toThrow(ForbiddenException);
  });

  it("listarGlobal de admin trae liquidaciones de todas las carteras con carteraNombre", async () => {
    (liquidacionRepo.find as jest.Mock).mockResolvedValue([
      { ...liquidacionFixture(), cartera: { id: 1, nombre: "Cartera Centro" } },
    ]);

    const result = await service.listarGlobal(adminContext);

    expect(liquidacionRepo.find).toHaveBeenCalledWith(
      expect.objectContaining({ where: {}, relations: { cartera: true }, order: { fecha: "DESC" } }),
    );
    expect(result).toHaveLength(1);
    expect(result[0].carteraNombre).toBe("Cartera Centro");
  });

  it("listarGlobal de un propietario solo trae liquidaciones de sus carteras", async () => {
    (liquidacionRepo.find as jest.Mock).mockResolvedValue([]);
    ...
    expect(liquidacionRepo.find).toHaveBeenCalledWith(
      expect.objectContaining({
        where: { cartera: { propietario: { id: 7 } } },
        relations: { cartera: true },
      }),
    );
  });

  it("exportar lanza NotFoundException si la liquidación no existe en la cartera", async () => {
    (carteraRepo.findOne as jest.Mock).mockResolvedValue(carteraFixture());
  ...
  it("un propietario no puede exportar una liquidación de cartera ajena -> 403", async () => {
    (carteraRepo.findOne as jest.Mock).mockResolvedValue(carteraFixture({ propietarioId: 2 }));

    await expect(service.exportar(1, 10, propietarioContext)).rejects.toThrow(ForbiddenException);
  });
