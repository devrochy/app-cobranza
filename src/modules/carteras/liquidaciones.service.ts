...
export class LiquidacionesService {
  ...
  async generar(
    ...
    const saved = await this.dataSource.transaction(async (manager) => {
      ...
      const cajaActual = await cajaRepo.findOne({ where: { cartera: { id: carteraId } } });
      const cajaAnterior = ultima?.cajaActual ?? caja.saldoInicial;

      const totales = await this.calcularTotales(carteraId, inicio, fin, manager);
      const {
        estimadoACobrar,
        sumaCartera,
        totalCobradoPeriodo,
      ...
      const pagos = await this.marcarLiquidados(carteraId, inicio, fin, manager);
  ...
  private async marcarLiquidados(
    ...
    const pagos = await pagoRepo.find({
      where: {
        ...
        cuota: { prestamo: { cartera: { id: carteraId } } },
      },
  ...
  async listarGlobal(
    ...
    const where =
      requester.rol === "propietario" ? { cartera: { propietario: { id: requester.sub } } } : {};
    const filas = await this.liquidacionRepo.find({
      where,
      relations: { cartera: true },
      order: { fecha: "DESC" },
    });
    return filas.map((l) => ({
      ...this.toPublic(l),
      carteraNombre: l.cartera.nombre,
    }));
  }
  ...
  async exportar(
    ...
    const l = await this.liquidacionRepo.findOne({
      where: { id: liquidacionId, cartera: { id: carteraId } },
    });
    if (!l) {
      throw new NotFoundException("La liquidación no existe en esta cartera");
    }
   ...
   * abonos del día). Se usa desde la APK (gestor) para descargar/compartir.
   */
  async exportarPdf(
    carteraId: number,
    liquidacionId: number,
    ...
    const base = new Date(`${l.fecha}T00:00:00`);
    const detalle = await this.obtenerDetalle(carteraId, this.inicioDelDia(base), this.finDelDia(base));
    ...
    doc.fontSize(16).text(`Liquidación ${cartera.nombre}`, { align: "center" });
    doc.moveDown(0.4);
  ...
  /** Detalle de pagos/abonos de una cartera dentro de una ventana (sin marcar). */
  ...
  private async sumaAbonos(carteraId: number, manager?: EntityManager): Promise<number> {
    const row = await this.qb(manager)
  ...
  private async sumaPagos(carteraId: number, inicio: Date, fin: Date, manager?: EntityManager): Promise<number> {
    // Solo pagos de cuotas atribuibles a la cartera (vía cuota → préstamo).
  ...
  private async sumaPrestamos(carteraId: number, inicio: Date, fin: Date, manager?: EntityManager): Promise<number> {
    const row = await this.qb(manager)
  ...
  private async sumaGastosAprobados(carteraId: number, inicio: Date, fin: Date, manager?: EntityManager): Promise<number> {
    const row = await this.qb(manager)
      .select("COALESCE(SUM(g.valor), 0)", "total")
      .from("gastos", "g")
      .where("g.cartera_id = :carteraId", { carteraId })
  ...
  private async sumaInyeccionesActivas(carteraId: number, inicio: Date, fin: Date, manager?: EntityManager): Promise<number> {
    const row = await this.qb(manager)
      .select("COALESCE(SUM(i.valor), 0)", "total")
      .from("inyecciones", "i")
      .where("i.cartera_id = :carteraId", { carteraId })
  ...
  async calcularTotales(
    ...
    const estimadoACobrar = await this.sumaCuotasPendientes(carteraId, inicio, fin, manager);
    const sumaAbonos = await this.sumaAbonos(carteraId, manager);
    const sumaCartera = Math.max(0, estimadoACobrar - sumaAbonos);
    const totalCobradoPeriodo = await this.sumaPagos(carteraId, inicio, fin, manager);
    const totalCobradoDia = await this.sumaPagos(carteraId, this.inicioDelDia(fin), this.finDelDia(fin), manager);
    const totalPrestado = await this.sumaPrestamos(carteraId, inicio, fin, manager);
    const totalGastos = await this.sumaGastosAprobados(carteraId, inicio, fin, manager);
    const totalInyeccion = await this.sumaInyeccionesActivas(carteraId, inicio, fin, manager);
    return {
      estimadoACobrar,
      sumaCartera,
      totalCobradoPeriodo,
