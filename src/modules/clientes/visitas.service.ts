...
export class VisitasService {
  ...
  async registrar(
    ...
    const prestamo = await this.prestamoRepo.findOne({
      where: { id: input.prestamoId, cartera: { id: carteraId }, cliente: { id: input.clienteId } },
    });
    if (!prestamo) {
      throw new NotFoundException("El préstamo principal no existe en esta cartera");
    }
    ...
    const creador: PromesaCreador = requester.rol === "gestor" ? "gestor" : "agente";
    ...
    const visitaId = await this.dataSource.transaction(async (manager) => {
      ...
      const visita = visitaRepo.create({
        cartera: { id: carteraId } as Visita["cartera"],
        carteraId,
