...
describe("CuotaService", () => {
  ...
  it("lanza NotFoundException si la cartera no existe al editar", async () => {
  ...
  it("un propietario no puede editar una cuota en una cartera ajena -> 403", async () => {
    (carteraRepo.findOne as jest.Mock).mockResolvedValue(carteraFixture({ propietarioId: 2 }));

    await expect(
      service.editarCuota(1, 10, { valorEsperado: 100 }, { password: "x", motivo: "m" }, propietarioContext),
