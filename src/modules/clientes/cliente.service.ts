...
export class ClienteService {
      ...
      const saved = await this.dataSource.transaction(async (manager) => {
        ...
        for (const evidencia of evidencias) {
          const nueva = evidenciaRepo.create({
            ...
            tipo: evidencia.tipo,
            carteraArchivo: evidencia.archivo.path,
    ...
    try {
      ...
      for (const evidencia of evidencias) {
        ...
        if (!existente) {
          ...
          existente.carteraArchivo = evidencia.archivo.path;
  ...
  async actualizar(
    ...
    const cliente = await this.repo.findOne({ where: { id: clienteId, cartera: { id: carteraId } } });
    if (!cliente) {
  ...
  async decidirPropuesta(
    ...
    const cambio = await this.cambioRepo.findOne({
      where: { id: cambioId, cliente: { cartera: { id: carteraId } } },
      relations: { cliente: true },
    });
    if (!cambio) {
      throw new NotFoundException("La propuesta no existe en esta cartera");
    }
  ...
  private async puedeEditar(requester: RequesterCarteraContext): Promise<boolean> {
    ...
    return this.permisosPropietario.tienePermiso(requester.sub, "actualizar_cliente");
  }
  ...
  async listar(carteraId: number, requester: RequesterCarteraContext): Promise<ClientePublic[]> {
    const cartera = await this.carteraRepo.findOne({ where: { id: carteraId } });
    if (!cartera) {
      throw new NotFoundException("La cartera no existe");
    }
  ...
  private async fotosFacialesDeClientes(
    ...
    for (const e of evidencias) {
      if (!map.has(e.clienteId)) {
        map.set(e.clienteId, urlArchivoServible(e.carteraArchivo) ?? e.carteraArchivo);
      }
    }
    return map;
  }
  ...
  async listarGlobal(
    ...
    const qb = this.repo
      ...
      .innerJoinAndSelect("cliente.cartera", "cartera")
    ...
    return {
      items: clientes.map((cliente) => ({
        ...this.toPublic(cliente, cliente.carteraId),
  ...
  private toPublic(cliente: Cliente, carteraId: number, fotoUrl: string | null = null): ClientePublic {
