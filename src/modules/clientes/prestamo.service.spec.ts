...
describe("PrestamoService", () => {
  ...
  function configFixture(overrides: Partial<CarteraConfig> = {}): CarteraConfig {
    return { ...CarteraConfigDefaults, ...overrides } as CarteraConfig;
  }
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        { provide: getRepositoryToken(CarteraConfig), useValue: configRepo },
  ...
  describe("validaciones", () => {
    ...
    it("lanza NotFoundException si el cliente no existe o es de otra cartera", async () => {
      (carteraRepo.findOne as jest.Mock).mockResolvedValue(carteraFixture());
    ...
    it("un propietario no puede registrar en una cartera ajena -> 403", async () => {
      ...
      await expect(service.crear(1, baseInput, propietarioContext, fechaOtorgado)).rejects.toThrow(
        ForbiddenException,
      );
    });
  ...
  describe("generación de cuotas", () => {
    it("genera numCuotas cuotas con valores coherentes y vencimientos espaciados", async () => {
      ...
      const totalEsperado = 1000 * 1.2; // 20% de la cartera
    ...
    it("usa el tipoInteres de la cartera por defecto", async () => {
      setupFeliz();
  ...
  describe("listarPorCliente", () => {
    ...
    it("lista los préstamos del cliente con sus cuotas", async () => {
      (carteraRepo.findOne as jest.Mock).mockResolvedValue(cartera);
      ...
      expect(prestamoRepo.find).toHaveBeenCalledWith(expect.objectContaining({
        where: { cartera: { id: 10 }, cliente: { id: 1 } },
        relations: { cuotas: true },
      }));
      expect(res[0].cuotas[0].valorEsperado).toBe(150);
    });

    it("lanza 404 si el cliente no pertenece a la cartera", async () => {
      (carteraRepo.findOne as jest.Mock).mockResolvedValue(cartera);
      (clienteRepo.findOne as jest.Mock).mockResolvedValue(null);
