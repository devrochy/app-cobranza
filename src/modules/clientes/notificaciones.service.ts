 ...
 * config por cartera (dias_anticipacion_notificacion, aviso_dia_cobro,
...
export class NotificacionesService {
  ...
  async ejecutarRecordatorios(input: EjecutarRecordatoriosInput): Promise<number> {
    ...
    const config = await this.configRepo.findOne({ where: { cartera: { id: input.carteraId } } });
    ...
    const cuotas = await this.cuotaRepo.find({
      where: {
        ...
        prestamo: { cartera: { id: input.carteraId } },
      },
  ...
  async ejecutarAvisoDiaCobro(carteraId: number, input: EjecutarConFechaInput = {}): Promise<number> {
    const hoy = input.hoy ?? new Date();
    ...
    const cuotas = await this.cuotaRepo.find({
      where: { estatus: "pendiente", fechaVencimiento: hoyStr, prestamo: { cartera: { id: carteraId } } },
      relations: { prestamo: { cliente: true } },
    });
  ...
  async ejecutarAlertaMora(carteraId: number, input: EjecutarConFechaInput = {}): Promise<number> {
    const hoy = input.hoy ?? new Date();
    const config = await this.configRepo.findOne({ where: { cartera: { id: carteraId } } });
    const umbral = config?.umbralMoraNotificacion ?? 0;

    const cuotas = await this.cuotaRepo.find({
      where: { estatus: "atrasada", prestamo: { cartera: { id: carteraId } } },
      relations: { prestamo: { cliente: true } },
    });
