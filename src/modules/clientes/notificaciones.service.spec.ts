...
describe("NotificacionesService", () => {
  ...
  it("envía recordatorio para cuotas que vencen dentro de N días y deduplica", async () => {
    ...
    mockConfigRepo.findOne.mockResolvedValue({ diasAnticipacionNotificacion: 3 } as CarteraConfig);

    const enviadas = await service.ejecutarRecordatorios({ carteraId: 1, hoy });
  ...
  it("crea la conversación si el cliente no tiene una activa", async () => {
    ...
    await service.ejecutarRecordatorios({ carteraId: 1, hoy });
