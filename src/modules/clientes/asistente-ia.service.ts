...
export class AsistenteIaService {
    ...
    if (!proxima) {
      return {
        contenido: construirTextoConsultaSaldo(
          ...
          await this.monedaCartera(cliente),
          0,
          null,
        ),
  ...
  private async computarSaldo(
    ...
    const moneda = await this.monedaCartera(cliente);
  ...
  private async monedaCartera(cliente: Cliente): Promise<string> {
    const cartera = await this.carteraRepo.findOne({ where: { id: cliente.carteraId } });
    return cartera?.moneda ?? "";
  }
