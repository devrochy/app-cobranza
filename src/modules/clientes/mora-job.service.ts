...
export class MoraJobService {
  ...
  async ejecutar(hoy: Date = new Date()): Promise<number> {
    ...
    const vencidas = await this.cuotaRepo.find({
      ...
      relations: { prestamo: { cliente: true, cartera: true } },
    });
    ...
    await this.dataSource.transaction(async (manager) => {
      ...
      // HU-13: recalcula el color de los clientes afectados (una vez por cliente/cartera).
      const procesados = new Set<string>();
      for (const cuota of marcadas) {
        const clienteId = cuota.prestamo?.cliente?.id;
        const carteraId = cuota.prestamo?.cartera?.id;
        if (!clienteId || !carteraId) {
          continue;
        }
        const clave = `${clienteId}:${carteraId}`;
