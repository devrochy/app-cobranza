...
describe("DetalleCuotaService", () => {
  ...
  const gestorContext = { rol: "gestor" as const, sub: 7 };
  ...
  it("devuelve el detalle de la cuota con pagos y saldo", async () => {
    ...
    const resultado = await service.obtener(1, 5, 51, gestorContext);
  ...
  it("lanza NotFound si la cartera no existe", async () => {
    ...
    await expect(service.obtener(1, 5, 51, gestorContext)).rejects.toThrow(
      NotFoundException,
    );
  });
  ...
  it("lanza Forbidden si el gestor no pertenece a la cartera", async () => {
