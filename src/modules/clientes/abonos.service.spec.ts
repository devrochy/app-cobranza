...
describe("AbonosService", () => {
  ...
  it("un propietario no puede abonar en una cartera ajena -> 403", async () => {
    (carteraRepo.findOne as jest.Mock).mockResolvedValue(carteraFixture({ propietarioId: 2 }));

    await expect(
      service.registrarAbono(1, { prestamoId: 20, valor: 50, metodoPago: "efectivo" }, propietarioContext),
    ).rejects.toThrow(ForbiddenException);
  });

  it("lanza NotFoundException si el préstamo no existe o no pertenece a la cartera", async () => {
  ...
  it("lanza NotFoundException si el abono no existe en la cartera", async () => {
    (carteraRepo.findOne as jest.Mock).mockResolvedValue(carteraFixture());
    (mockReautenticacion.validar as jest.Mock).mockResolvedValue(undefined);
