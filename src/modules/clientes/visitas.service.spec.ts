...
describe("VisitasService", () => {
  ...
  it("un propietario no puede registrar una visita en una cartera ajena -> 403", async () => {
    (carteraRepo.findOne as jest.Mock).mockResolvedValue(carteraFixture({ propietarioId: 2 }));

    await expect(
      service.registrar(1, { prestamoId: 20, clienteId: 5, resultado: "no_pago", motivoNoPago: "no_esta" }, propietarioContext),
    ).rejects.toThrow(ForbiddenException);
  });

  it("lanza NotFoundException si el cliente no existe o no pertenece a la cartera", async () => {
  ...
  it("lanza NotFoundException si el préstamo principal no existe o no pertenece a la cartera", async () => {
    (carteraRepo.findOne as jest.Mock).mockResolvedValue(carteraFixture());
