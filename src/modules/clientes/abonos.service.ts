...
export class AbonosService {
  ...
  async registrarAbono(
    ...
    const prestamo = await this.prestamoRepo.findOne({
      where: { id: input.prestamoId, cartera: { id: carteraId } },
      relations: { cliente: true },
    });
    if (!prestamo || prestamo.estatus !== "vigente") {
      throw new NotFoundException("El préstamo no existe o no está vigente en esta cartera");
    }
  ...
  async eliminarAbono(
    ...
    const abono = await this.abonoRepo.findOne({ where: { id: abonoId, prestamo: { cartera: { id: carteraId } } } });
    if (!abono) {
      throw new NotFoundException("El abono no existe en esta cartera");
    }
    if (abono.liquidado) {
