...
describe("Negociación de abono parcial o refinanciación por WhatsApp (e2e, HU-29)", () => {
  ...
  beforeAll(async () => {
    ...
    process.env.JWT_REFRESH_SECRET = "test-refresh-secret-negocia";
  ...
  it("registra un acuerdo de abono parcial con tipo abono_parcial y confirma", async () => {
    const res = await request(app.getHttpServer())
      .post("/whatsapp/simulado/recibir")
      .set("Authorization", `Bearer ${accessTokenAdmin}`)
      .send({ conversacionId, contenido: "puedo abonar 100 el viernes" });

    expect(res.status).toBe(201);

    const promesa = await promesaRepo.findOne({
      where: { prestamo: { id: prestamoId }, tipo: "abono_parcial", creadoPor: "ia" },
      order: { id: "DESC" },
    });
    expect(promesa).toBeDefined();
    expect(promesa?.estado).toBe("pendiente");
    expect(promesa?.valorPrometido).toBe(100);
    ...
    expect(respuesta?.contenido.toLowerCase()).toContain("abono");
  });

  it("registra un acuerdo de refinanciación con tipo refinanciacion y confirma", async () => {
    const res = await request(app.getHttpServer())
      .post("/whatsapp/simulado/recibir")
      .set("Authorization", `Bearer ${accessTokenAdmin}`)
      .send({ conversacionId, contenido: "quiero refinanciar el viernes" });

    expect(res.status).toBe(201);

    const promesa = await promesaRepo.findOne({
      where: { prestamo: { id: prestamoId }, tipo: "refinanciacion", creadoPor: "ia" },
      order: { id: "DESC" },
    });
    expect(promesa).toBeDefined();
    expect(promesa?.estado).toBe("pendiente");
    ...
    expect(respuesta).toBeDefined();
    expect(respuesta?.contenido.toLowerCase()).toContain("refinanciación");
  });
});