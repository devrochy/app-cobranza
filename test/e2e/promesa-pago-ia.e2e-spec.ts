...
describe("Registro de promesa de pago por WhatsApp (e2e, HU-28)", () => {
  ...
  const PASSWORD = "Propietario#Promesa2026";
  ...
  beforeAll(async () => {
    ...
    await cuotaRepo.createQueryBuilder().delete().where("prestamo_id IN (SELECT id FROM prestamos WHERE cartera_id IN (SELECT id FROM carteras WHERE nombre = 'Cartera PROMESA'))").execute();
    await prestamoRepo.createQueryBuilder().delete().where("cartera_id IN (SELECT id FROM carteras WHERE nombre = 'Cartera PROMESA')").execute();
    await clienteRepo.createQueryBuilder().delete().where("cartera_id IN (SELECT id FROM carteras WHERE nombre = 'Cartera PROMESA')").execute();
    await carteraRepo.delete({ nombre: "Cartera PROMESA" });
    await gestorRepo.delete({ codigo: "CB-PROMESA-1" });
    await propietarioRepo.delete({ codigo: "SC-PROMESA-1" });
    ...
    const propietario = await propietarioRepo.save({
      usuario: "propietario-promesa-1",
      passwordHash: await bcrypt.hash(PASSWORD, 4),
      nombre: "S",
      apellido: "E2E",
      correo: "propietario-promesa-1@correo.com",
    ...
    const gestor = await gestorRepo.save({
      ...
      usuario: "gestor-promesa-1",
      passwordHash: await bcrypt.hash(PASSWORD, 4),
      nombre: "C",
      apellido: "E2E",
      correo: "gestor-promesa-1@correo.com",
