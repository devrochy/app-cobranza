...
describe("Detección y derivación a agente humano (e2e, HU-32)", () => {
  ...
  const PASSWORD = "Socio#Deriv2026";
  ...
  beforeAll(async () => {
    ...
    await cuotaRepo.createQueryBuilder().delete().where("prestamo_id IN (SELECT id FROM prestamos WHERE ruta_id IN (SELECT id FROM rutas WHERE nombre = 'Ruta DERIV'))").execute();
    await prestamoRepo.createQueryBuilder().delete().where("ruta_id IN (SELECT id FROM rutas WHERE nombre = 'Ruta DERIV')").execute();
    await clienteRepo.createQueryBuilder().delete().where("ruta_id IN (SELECT id FROM rutas WHERE nombre = 'Ruta DERIV')").execute();
    await rutaRepo.delete({ nombre: "Ruta DERIV" });
    await cobradorRepo.delete({ codigo: "CB-DERIV-1" });
    await socioRepo.delete({ codigo: "SC-DERIV-1" });
    ...
    const socio = await socioRepo.save({
      usuario: "socio-deriv-1",
      passwordHash: await bcrypt.hash(PASSWORD, 4),
      nombre: "S",
      apellido: "E2E",
      correo: "socio-deriv-1@correo.com",
    ...
    const cobrador = await cobradorRepo.save({
      socio: { id: socio.id },
      usuario: "cobrador-deriv-1",
      passwordHash: await bcrypt.hash(PASSWORD, 4),
      nombre: "C",
      apellido: "E2E",
      correo: "cobrador-deriv-1@correo.com",
