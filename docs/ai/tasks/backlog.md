...
- Fecha: 2026-08-12
- Descripción: `ruta-config.service.ts` replica el `assertOwned` de `rutas.service.ts` (misma lógica socio-sobre-sus-rutas). Con `inyecciones.service.ts` ya son 4 usos (rutas, ruta-config, inyecciones) más los de socios/cobradores. Conviene extraer un helper compartido. También se duplica el `numericTransformer` (ruta, ruta-config, inyeccion).
