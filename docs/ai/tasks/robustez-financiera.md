...
- Comandos ejecutados para verificar: `bash scripts/check.sh` (107 suites, 957 tests) + `npm run test:e2e` (57 suites, 407 tests) → verde.
- Archivos modificados:
  ...
  - `src/modules/cartera/pagos.service.ts` (+ `.spec.ts`) — UPDATE condicional de cuota.
  - `src/modules/cartera/abonos.service.ts` (+ `.spec.ts`) — deuda en transacción + lock sobre préstamo.
