...
- [x] Bloque 4: `scripts/check.sh` verde + commit/PR.
...
- Comandos ejecutados para verificar: `bash scripts/check.sh` (lint + typecheck + tests) verde.
- Archivos modificados: `src/config/cors.ts` (+ `.spec.ts`, 4 tests), `src/main.ts`, `.env.example`.
- Mergeado a `develop` vía PR #81.
