...
- **Resuelto (tipado preparatorio de cartera):** la extensión de `RequesterContext` → `RolUsuario` en `src/modules/cartera/{cliente,prestamo}.service.ts` y el mock de `DataSource` en `src/modules/cartera/cartera.controller.spec.ts` quedaron aplicados en `feature/registrar-prestamo` (commit `ac3f1dd`, PR #17), NO en esta rama, porque esos archivos solo existen ahí. Cuando la PR #17 se mergee a `develop`, el tipado de cartera será consistente con el `RolUsuario` ampliado de esta rama.
...
- **Revisión final (code-reviewer, 2026-08-17):** APROBADO CON OBSERVACIONES (sin bloqueantes). Observaciones registradas en backlog: falta test de rol desconocido en el guard, `refresh` no ramifica el rol cobrador (futuro), y verificar que toda ruta con `JwtAuthGuard` tenga también `PermisoGuard` antes de habilitar el login del cobrador.